...
# now with a closure used in conditionals and vars
$count = 0;
...
# now a code ref that returns different things each time it's called
$count = 0;
...
$template->param(loop_one => sub { [{var => 1}, {var => 2}] });
$output = clean($template->output);
is($output, '1 2', 'simple coderef used for a loop and conditional');

# now a code ref (for a loop also used as a conditional) that returns different things each time it's called
$count = 1;
...
$template->param(loop_one => sub { [{var => $count++}, {var => $count++}] });
$output = clean($template->output);
is($output, '3 4', 'loop coderef that is also used in a conditional gets called correct number of times');
is($count, 5, 'currect number of increments');

# now a closure for a non-existant loop
$count = 0;
...
# now a closure for a loop that should never be executed
