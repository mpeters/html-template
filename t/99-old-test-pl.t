...
my @result;
eval { @result = $template->query(loop => ['EXAMPLE_LOOP', 'BEE']); };

ok(       $@ =~ /error/
      and $template->query(name => 'var')          eq 'VAR'
      and $template->query(name => 'EXAMPLE_LOOP') eq 'LOOP'
      and exists $params{bee}
      and exists $params{bop}
      and exists $params{example_inner_loop}
      and $template->query(name => ['EXAMPLE_LOOP', 'EXAMPLE_INNER_LOOP']) eq 'LOOP');
...
$output = $template->output;
ok(       $output !~ /ADVERB/i
      and $template->param('ADVERB') eq 'painfully'
      and $template->param('adverb') eq 'very'
      and $output =~ /very painfully/);
...
ok(       $output !~ /ADVERB/i
      and $template->param('ADVERB') eq 'painfully'
      and $template->param('adverb') eq 'painfully'
      and $output =~ /painfully painfully/);
...
$template->param(
    outer_loop => [
        {
            foo        => 'foo val 1',
            inner_loop => [{bar => 'bar val 1'}, {bar => 'bar val 2'},],
        },
        {
            foo        => 'foo val 2',
            inner_loop => [{bar => 'bar val 3'}, {bar => 'bar val 4'},],
        }
    ]
);
$output = $template->output;
ok($output =~ /inner loop foo:    foo val 1/ and $output =~ /inner loop foo:    foo val 2/);
...
eval { my $template = HTML::Template->new(scalarref => \'<tmpl_var>'); };
ok($@ =~ /No NAME given/);
...
# test empty filename
eval { $template = $template = HTML::Template->new(path => ['templates'], filename => ''); };
